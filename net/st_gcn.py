...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        #
        # if clas.size(0) != 0:
        #     loss = self.update_classavg(x, clas)
        #
        # secloss = 0
        #
        # for i,y in enumerate(x):
        #     y = torch.repeat_interleave(y.unsqueeze(dim = 0), repeats = 70, dim = 0)
        #     try:
        #         loss -= F.mse_loss(self.clsavg, y) / 70
        #         secloss = 1
        #     except:
        #         loss = 0
        #         secloss = 0
        #     yy = torch.cat((self.clsavg,y),dim = 1)
        #     if i == 0:
        #         z = yy
        #     else:
        #         z = torch.cat((z,yy),dim=0)
        # z = torch.sigmoid(self.fcn(z))
        # z = z.view(batchnum, -1)
        # if secloss:
        #     return z, loss
        # else:
